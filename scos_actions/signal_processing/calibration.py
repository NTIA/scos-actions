...
def y_factor(pwr_noise_on_watts: ndarray, pwr_noise_off_watts: ndarray,
             enr_linear: float, enbw_hz: float,
             temp_kelvins: float = 300.) -> Tuple[float, float]:
    """
    ...
    noise_factor = enr_linear / (y - 1.)
    gain_watts = pwr_noise_on_watts / (Boltzmann * temp_kelvins * enbw_hz
                                       * (enr_linear + noise_factor))
