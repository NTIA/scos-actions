...
def test_get_iterable_parameters_no_lists(valid_params_no_lists):
    ...
    assert all(
        i_params[0][k] == valid_params_no_lists[k] for k in valid_params_no_lists.keys()
    )
...
def test_get_iterable_parameters_all_lists():
    ...
    assert (
        i_params[0]["sample_rate"]
        == all_lists["sample_rate"][all_lists["frequency"].index(minf)]
    )
    assert i_params[-1]["frequency"] == maxf
    assert (
        i_params[-1]["sample_rate"]
        == all_lists["sample_rate"][all_lists["frequency"].index(maxf)]
    )
