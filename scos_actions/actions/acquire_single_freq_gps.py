...
class SingleFrequencyGPSAcquisition(Action):
    ...
    def __init__(self, name, frequency, gain, sample_rate, duration_ms, radio):
        super(SingleFrequencyGPSAcquisition, self).__init__()
    ...
    def acquire_data(self, epoch_time):
        msg = "At GPS time {}, acquiring {} samples at center freq. {} MHz"
        ...
        logger.debug(msg.format(epoch_time, nsamps, frequency / 1e6))

        # Drop ~10 ms of samples
        #nskip = int(0.01 * sample_rate)
        logger.debug("in acquire_data; radio struct: ".format(self.radio))
        data = self.radio.acquire_gps_samples(nsamps, epoch_time)#, nskip=nskip)
