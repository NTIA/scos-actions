...
def check_metadata_fields(
    metadata, entry_name, action_name, task_id, recording=None
):
    ...
    assert (
        metadata["global"]["ntia-scos:action"]["name"]
        == action_name
    )
    ...
    assert (
        metadata["global"]["ntia-scos:task"]
        == task_id
    )
    if recording:
        assert "ntia-scos:recording" in metadata["global"]
        assert (
            metadata["global"]["ntia-scos:recording"]
            == recording
        )
    else:
    ...
    assert metadata["global"]["ntia-core:measurement"][
        "frequency_tuned_low"
    ]
    assert metadata["global"]["ntia-core:measurement"][
