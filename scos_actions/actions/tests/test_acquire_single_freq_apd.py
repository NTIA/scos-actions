...
from scos_actions.actions.tests.test_acquire_single_freq_fft import SINGLE_FREQUENCY_FFT_ACQUISITION
...
SINGLE_FREQUENCY_APD_ACQUISITION = {
    "name": "test_acq",
    "start": None,
    "stop": None,
    "interval": None,
    "action": "test_single_frequency_apd_action",
}
...
def test_detector():
    ...
    action = actions["test_single_frequency_apd_action"]
    assert action.description
    action(SINGLE_FREQUENCY_APD_ACQUISITION, 1)
    assert _task_id == 1
    assert _data.any()
    assert _metadata
    check_metadata_fields(
        _metadata,
        SINGLE_FREQUENCY_APD_ACQUISITION["name"],
        SINGLE_FREQUENCY_APD_ACQUISITION["action"],
        1,
    )
